  ...
  >
    ...
    <template #header-buttons>
      <Button
        ...
        color="primary"
        class="mr-2"
        @click="visit('posts.create')"
      />
    </template>

    <!-- Custom Cell Templates -->
    <template #record.tags="{ record }">
      <Tag
        v-for="tag in record"
        :key="tag.id"
        :value="tag.name.en"
    ...
    <template #data-buttons="{ record }">
      <li class="p-menuitem">
        <div class="p-menuitem-content">
          <a
            @click="visit('posts.edit', { post: record.id })"
...
const columns = [
  ...
  { field: "tags", header: "Tags", headerStyle: "width:8%", sortable: false },
  {
