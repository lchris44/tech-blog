...
(function () {
    ...
    var createLink = function (editor, selectedElm, text, linkAttrs) {
      if(linkAttrs.href && linkAttrs.href.toString().includes('#')) {
        linkAttrs.class = 'scroll-link';
      }
      if (isImageFigure(selectedElm)) {
    ...
    var init = function (initialData, linkCatalog) {
      ...
      var getTitleFromUrlChange = function (url) {
        return someIf(persistentData.title.length <= 0, Optional.from(url.meta ? url.met.title : null).getOr(''));
      };
      var getTextFromUrlChange = function (url) {
        return someIf(persistentData.text.length <= 0, Optional.from(url.meta ? url.meta.text : null).getOr(url.value));
      };
    ...
    var getAnchors = function (editor) {
      var anchorNodes = editor.dom.select('a:not([href]):not(.toc)');
